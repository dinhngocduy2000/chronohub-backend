...
class AuthService:
    ...
    def get_sso_auth_url(self, ctx: AppContext) -> Tuple[str, str]:
        """
        Build the SSO OAuth 2.0 authorization URL for redirect-based sign-in.
        Returns (url, state). The handler should set state in a cookie and return the URL to the frontend.
        """
        url, state = self.sso_strategy.get_auth_url(ctx)
    ...
    async def login_with_sso_callback(
        ...
        if idinfo is None:
            ...
            raise Exception("SSO Not validated")
